...
extern char*  yytext;
extern char   pdev[30];
extern LOGSTRUCT detect[]; extern int detectindex;
extern CLKSTRUCT clocks[]; extern int clocksindex;
  ...
  for(i=0;i<detectindex;i++) {
   ...
   if (strcmp(key,"VESA")  == 0) {
    ...
    switch(msg[n].vmodes[k].x) {
     /*640x480...*/
     case 640 :
      if ((vsync > 50) && (vsync <= 60))  msg[n].vmodes[k].hsync = 32; 
      if ((vsync > 60) && (vsync <= 90))  msg[n].vmodes[k].hsync = 35;
      CheckRange(msg,n,k);
      break;

     /*800x600...*/
     case 800 :
      if ((vsync > 50) && (vsync <= 60))  msg[n].vmodes[k].hsync = 35;
      if ((vsync > 60) && (vsync <= 90))  msg[n].vmodes[k].hsync = 45;
      CheckRange(msg,n,k);
      break;

     /*1024x768...*/
     case 1024:
      if ((vsync > 50) && (vsync <= 60))  msg[n].vmodes[k].hsync = 45;
      if ((vsync > 60) && (vsync <= 90))  msg[n].vmodes[k].hsync = 62;
      CheckRange(msg,n,k);
      break; 

     /*1152x864...*/
     case 1152:
      if ((vsync > 50) && (vsync <= 70))  msg[n].vmodes[k].hsync = 62;
      if ((vsync > 70) && (vsync <= 90))  msg[n].vmodes[k].hsync = 72;
      CheckRange(msg,n,k);
      break;

     /*1280x1024...*/
     case 1280:
      if ((vsync > 50) && (vsync <= 80))  msg[n].vmodes[k].hsync = 70;
      if ((vsync > 80) && (vsync <= 100)) msg[n].vmodes[k].hsync = 80;
      CheckRange(msg,n,k);
      break;

     /*1600x1200...*/
     case 1600:
      if ((vsync > 50) && (vsync <= 80))  msg[n].vmodes[k].hsync = 85;
      if ((vsync > 80) && (vsync <= 100)) msg[n].vmodes[k].hsync = 95;
      CheckRange(msg,n,k);
      break;
