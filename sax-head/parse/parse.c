...
XF86ConfigPtr ReadConfigFile (char *filename) {
 ...
 char CONFPATH[SIZE]="%A,%R,/etc/%R,%P/etc/X11/%R,%E,%F,/etc/X11/%F," \
 ...
 for (lp;lp;lp=lp->list.next) {
  ...
  if ((ap != NULL) && (ap->adj_where == -1)) {
   ...
   for (ap;ap;ap=ap->list.next) {
    ...
    strcpy(left,  "<none>");
    strcpy(right, "<none>");
    if (ap->adj_screen_str != NULL) strcpy(id,ap->adj_screen_str);
    if (ap->adj_scrnum     > 0)     num = ap->adj_scrnum;
    if ((ap->adj_top_str    != NULL)  && 
        (strcmp(ap->adj_top_str,"")    != 0)) strcpy(top,ap->adj_top_str);
    if ((ap->adj_bottom_str != NULL)  &&
        (strcmp(ap->adj_bottom_str,"") != 0)) strcpy(bottom,ap->adj_bottom_str);
    if ((ap->adj_left_str   != NULL)  &&
        (strcmp(ap->adj_left_str,"")   != 0)) strcpy(left,ap->adj_left_str);
    if ((ap->adj_right_str  != NULL)  &&
        (strcmp(ap->adj_right_str,"")  != 0)) strcpy(right,ap->adj_right_str);
    if (count == 0) 
     sprintf(line,"%s,%d,%s,%s,%s,%s",id,num,top,bottom,left,right);
    else
     sprintf(line,":%s,%d,%s,%s,%s,%s",id,num,top,bottom,left,right);
    count++;
    strcat(screen,line);
   ...
   for (ap;ap;ap=ap->list.next) {
    ...
    for (hp;hp;hp=hp->list.next) {
     if (strcmp(scid,hp->adj_screen_str) == 0) {
      switch(hp->adj_where) {
       case 1: {
        strcpy(right,hp->adj_refscreen);  break;
       } 
       case 2: {
        strcpy(left,hp->adj_refscreen);   break;
       }
       case 3: {
        strcpy(top,hp->adj_refscreen);    break;
       }
       case 4: {
        strcpy(bottom,hp->adj_refscreen); break;
       }
      }
     }
    }
    hp = (XF86ConfAdjacencyPtr) lp->lay_adjacency_lst;
    if (scount == 1) {
     sprintf(line,"%s,%d,%s,%s,%s,%s",scid,num,top,bottom,left,right);
    } else {
     sprintf(line,":%s,%d,%s,%s,%s,%s",scid,num,top,bottom,left,right); 
    }
    strcat(screen,line);
...
char* GetMonitorSpecs (XF86ConfigPtr conf) {
 ...
 for (mp;mp;mp=mp->list.next) {
  ...
  for (i = 0; i < mp->mon_n_hsync; i++) {
   sprintf(syncline, "%.5g-%.5g",mp->mon_hsync[i].lo,mp->mon_hsync[i].hi);
  }
  sprintf(line,"Hs=%s ",syncline);
  strcat(result,line);
  syncline[0] = '\0';
  for (i = 0; i < mp->mon_n_vrefresh; i++) {
   sprintf(syncline,"%.5g-%.5g",mp->mon_vrefresh[i].lo,mp->mon_vrefresh[i].hi);
  }
   ...
   for (lp;lp;lp=lp->list.next) {
    if (count == 0) {
     sprintf(line,"%s %4.2f %d %d %d %d %d %d %d %d",
       ...
       lp->ml_hdisplay,lp->ml_hsyncstart,lp->ml_hsyncend,lp->ml_htotal,
       lp->ml_vdisplay,lp->ml_vsyncstart,lp->ml_vsyncend,lp->ml_vtotal);
    } else {
     sprintf(line,",%s %4.2f %d %d %d %d %d %d %d %d",
       lp->ml_identifier, lp->ml_clock/1000.0,
       lp->ml_hdisplay,lp->ml_hsyncstart,lp->ml_hsyncend,lp->ml_htotal,
