...
DESCRIPTION   : xapi (X11-Application-Interface)
              : XPlot is a special widget used to show the
              : ServerLayout structure
              :
              :
...
//============================================
// XLayoutLine Constructor
//--------------------------------------------
XLayoutLine::XLayoutLine ( void ) {
	mLeft=mRight=mTop=mBottom = -1;
}

//============================================
// XMatrixPoint Matrix Constructor
//--------------------------------------------
XMatrixPoint::XMatrixPoint (int index) {
	mIndex = index;
}

//============================================
// XMatrixInfo Constructor
//--------------------------------------------
XMatrixInfo::XMatrixInfo ( void ) {
	mLeft=mRight=mTop=mBottom = -1;
}
...
//--------------------------------------------
void XMatrixPoint::setIndex (int index) {
	mIndex = index;
}
...
// get index number
//--------------------------------------------
int XMatrixPoint::getIndex (void) {
	return (mIndex);
}

//============================================
// XPlot Matrix view Constructor 
//--------------------------------------------
XPlot::XPlot (QWidget* parent,int length) : QWidget (parent) {
	setMinimumWidth  ( 300 );
	setMinimumHeight ( 1600 );
	mParent  = parent;
	mRect    = length;
	mPainted = false;
	mID      = 0;
...
//============================================
// XPlot matrix resized
//--------------------------------------------
void XPlot::resizeEvent (QResizeEvent*) {
	buildMatrix();
}
...
// paint matrix view
//--------------------------------------------
void XPlot::paintEvent (QPaintEvent *) {
	// ...
...
// build Matrix
//--------------------------------------------
void XPlot::buildMatrix ( void ) {
	// ...
	// create matrix. The XMatrixPoint list contains an 
	// XMatrixPoint object for each index containing
	...
	for (int y=mRect;y<=Height;y+=mRect) {
	for (int x=mRect;x<=Width;x+=mRect) {
		XMatrixPoint* p = new XMatrixPoint (index);
    ...
    // ----------------------------
	QIntDict<XFig> freeze = mWidget;
	QIntDictIterator<XFig> it (freeze);
	for (; it.current(); ++it) {
...
//--------------------------------------------
void XPlot::removeWidget ( int id ) {
	QList<XLayoutLine> layout = getLayout();
...
//--------------------------------------------
void XPlot::clear ( void ) {
	QIntDictIterator<XFig> it (mWidget);
	for (; it.current(); ++it) {
		delete (it.current());
...
//--------------------------------------------
XFig* XPlot::addWidget ( int r,int c,int id,const QString & tip ) {
	if (! mPainted) {
		buildMatrix();
	}
	int index = c + (r * mColumns);
	XMatrixPoint* p = mPoint.at (index);
	...
	XFig *figure = new XFig (this,x,y,index,id);
	figure -> setToolTip ( tip );
	QObject::connect(
		figure, SIGNAL  (sigReleased(XFig*)),
		this ,  SLOT    (gotReleased(XFig*))
    );
	QObject::connect(
...
//--------------------------------------------
void XPlot::setWidget (
	int id, int left,int right,int top,int bottom,const QString & tip
) {
	XMatrixInfo* widget = new XMatrixInfo;
	widget -> ID    = id;
...
//--------------------------------------------
void XPlot::setLayout ( int startRow, int startCol ) {
	unsigned int widgetCount = mWidget.count();
	QListIterator<XMatrixInfo> it (mWidgetStack);
	for (; it.current(); ++it) {
		XFig* figure = NULL;
		XMatrixInfo* current = it.current();
		// ...
		...
		// ---------------
		if (mWidget.isEmpty()) {
			addWidget (startRow,startCol,current->ID,current->mTip);
			widgetCount++;
			continue;
		}
		// ...
		...
		if (figure) {
			...
			addWidget (row,col,current->ID,current->mTip);
			widgetCount++;
			break;
		}
		}
...
//--------------------------------------------
XFig* XPlot::searchWidget ( int id ) {
	QIntDictIterator<XFig> it (mWidget);
	for (; it.current(); ++it) {
...
//--------------------------------------------
XFig* XPlot::searchWidget ( int x, int y ) {
	QIntDictIterator<XFig> it (mWidget);
	for (; it.current(); ++it) {
	QPoint pos = it.current()->getPosition();
	if ((pos.x() == x) && (pos.y() == y)) {
...
// search widget object at position [x,y]
//--------------------------------------------
XFig* XPlot::searchWidget ( QPoint p ) {
...
//--------------------------------------------
int XPlot::getPointIndex (int x,int y) {
	QListIterator<XMatrixPoint> it (mPoint);
	for (; it.current(); ++it) {
	XMatrixPoint* p = it.current();
...
//--------------------------------------------
void XPlot::gotPressed ( int id ) {
	// log (L_INFO,"XPlot::gotPressed(%d) called\n",id);
	sigFigure (id);
}
...
//--------------------------------------------
void XPlot::gotReleased ( XFig* draged ) {
	int nearest  = 0;
	int mindist  = width();
	...
	// ...
	// get the XMatrixPoint object for the nearest index and
	// compare the neighbours distances. Use the nearest
	// neighbour as drag location
	// ---------------------------
	XMatrixPoint *p = mPoint.at(nearest);
	...
	XFig* countWidgets [mWidget.count()];
	for (unsigned int i=0;i<mWidget.count();i++) {
		countNeighbours[i] = 0;
	}
	QIntDictIterator<XFig> i (mWidget);
	for (; i.current(); ++i) {
	...
	XMatrixPoint* matrixPoint = mPoint.at (oldindex);
	for (int l=0;l<2;l++) {
		QPoint env;
		switch (l) {
...
//--------------------------------------------
QList<XLayoutLine> XPlot::getLayout ( void ) {
	QList<XLayoutLine> layout;
	QIntDictIterator<XFig> it (mWidget);
	...
	// -------------------------------------
	for (i=0;i<count;i++) {
		XFig* widget  = searchWidget (IDs[i]);
		...
		XLayoutLine* env = new XLayoutLine;
		XFig* neighbour;
		for (int l=0;l<4;l++) {
		switch (l) {
